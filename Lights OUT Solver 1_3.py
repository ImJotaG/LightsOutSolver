...
def generar_entradas():
    if verificar_tamano_valido():
        ...
        for i in range(n):
            fila_botones = []
            for j in range(n):
                boton = ttk.Button(frame_botones, text="0", command=lambda i=i, j=j: cambiar_estado(i, j))
...
boton_verificar_tamano.grid(row=0, column=2)

var_toro = tk.IntVar()
var_toro.set(0)
toro_checkbox = tk.Checkbutton(ventana, text="Modo toro", variable=var_toro)
toro_checkbox.grid(row=1, column=0, columnspan=3)

frame_botones = tk.Frame(ventana)
frame_botones.grid(row=2, column=0, columnspan=3)

boton_generar_entradas = tk.Button(frame_botones, text="Generar Entradas", command=generar_entradas)
boton_generar_entradas.grid(row=0, column=0)

boton_ejecutar = tk.Button(frame_botones, text="Ejecutar Código", command=ejecutar_codigo_con_gui)
boton_ejecutar.grid(row=0, column=1)
